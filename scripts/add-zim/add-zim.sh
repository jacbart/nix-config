...
sudo runuser -u "${KIWIX_USER}" -- xh --follow --download "https://download.kiwix.org/zim/${PATH}${ZIM_FILE}" --output "${KIWIX_DIR}${PATH}${ZIM_FILE}"

sudo runuser -u "${KIWIX_USER}" -- kiwix-manage "${KIWIX_DIR}library.xml" add "${KIWIX_DIR}${PATH}${ZIM_FILE}"
